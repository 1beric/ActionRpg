...
public class Window implements Updateable, Stringable {
	...
	public Window(long window, int width, int height) {
		...
		this.callbacks().center().windowContext().vsync(true).show(true);
	...
	public Window center() {
		...
		GLFW.glfwSetWindowPos(m_Window, (vidmode.width() - m_Width) / 2,
	...
	public Window callbacks() {
		...
		GLFW.glfwSetWindowFocusCallback(m_Window, (_window, focused) -> {
			Application.instance().handleEvent(
					focused ? new WindowFocusEvent(this) : new WindowLostFocusEvent(this));
		});

		// MouseEnterWindowEvent and MouseExitWindowEvent
		GLFW.glfwSetCursorEnterCallback(m_Window, (_window, entered) -> {
			Application.instance().handleEvent(
					entered ? new MouseEnterWindowEvent(this) : new MouseExitWindowEvent(this));
		});
		...
		GLFW.glfwSetScrollCallback(m_Window, (_window, dx, dy) -> {
			Application.instance()
					.handleEvent(new MouseScrolledEvent(this, (float) dx, (float) dy));
		});
	...
	public String string(int indentAmt) {
		return StringTools.buildString(StringTools.indent(indentAmt), "Window: ", "" + m_Window,
				", (", "" + m_Width, ":", "" + m_Height, ")", m_Focused ? ", focused" : "");
	}
