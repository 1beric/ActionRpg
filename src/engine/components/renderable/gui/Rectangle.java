...
public class Rectangle extends Mesh2 {
	...
	public Rectangle(Vector2f horizontalBounds, Vector2f verticalBounds) {
		m_Rect = new Rect(horizontalBounds, verticalBounds);
	...
	public Rectangle(float width, float height) {
		m_Rect = new Rect(new Vector2f(0, width), new Vector2f(0, height));
	...
	public Rectangle color(Color color) {
		try {
			...
			String newName = MathTools.randomFilename();
			Material newMaterial = m_Rect.material().clone(newName);
			m_Rect.material(newMaterial);
			m_Rect.material().<ColorUniform>uniform("color").val(color);
		}
		return this;
	}
