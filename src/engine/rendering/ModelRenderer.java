...
public class ModelRenderer {
	...
	public static final float FOV = (float) (Math.PI / 2f);
	public static final float PERSPECTIVE_NEAR = 0.001f;
	public static final float PERSPECTIVE_FAR = 5000;

	public static void render(Layer layer) {
		start();
		float aspect = layer.window().width() * 1.0f / layer.window().height();

		Camera cam = layer.<Camera>component(Camera.class);
		Light light = layer.<Light>component(Light.class);
		Matrix4f projection = new Matrix4f().identity().perspective(FOV, aspect, PERSPECTIVE_NEAR,
				PERSPECTIVE_FAR);
	...
	public static void renderTransforms(Layer layer) {
		...
		if (selected != null)
			selected.<Transform>component(Transform.class).render(cam, light, projection);
	...
	public static Matrix4f perspective(float aspect) {
		return new Matrix4f().identity().perspective(FOV, aspect, PERSPECTIVE_NEAR,
				PERSPECTIVE_FAR);
	}
